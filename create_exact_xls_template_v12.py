...
# 设置列宽（根据用户截图调整，增加H列宽度以适应换行）
column_widths = [5, 12, 20, 12, 22, 10, 15, 25, 12, 12, 12, 12, 15, 8]
...
# 合并总体评价意见后面的H到N的格子，增加宽度以适应换行
ws.merge_cells('H19:N19')

# 合并课程组负责人签字后面的格子（H-N列），留出足够空间给电子签名
...
print('✅ 完全一致模板V12已创建: 期初资料/附件3 课程组教学资料检查情况记录表-完全一致模板.xlsx')