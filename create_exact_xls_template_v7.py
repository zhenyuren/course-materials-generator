...
# 总体评价意见后面的H到N的格子是合并的，显示固定文本
h19_cell = ws.cell(row=19, column=8, value='满足教学大纲要求，实施正常课程教学')
...
# 添加课程组负责人签字行（第20行）- 在表右边部分，无边框，靠最右（N列）
leader_cell = ws.cell(row=20, column=14, value='课程组负责人签字：{{teacherName}}')
...
# 添加日期行（第21行）- 在L列，日期后面的两个合并后，在M、N列
date_cell = ws.cell(row=21, column=12, value='日期：{{currentDate}}')
...
print('✅ 完全一致模板V7已创建: 期初资料/附件3 课程组教学资料检查情况记录表-完全一致模板.xlsx')